...
function renderInvitation(data) {
    ...
    try {
        ...
        if (giftContainer) {
            ...
            <p class="text-slate-700 text-2xl font-bold">${info.Name}</p>
            <p class="text-slate-700 text-lg">${info.Number}</p>
            <p class="text-slate-700 text-lg">${info.Address}</p>
            <button onclick="copyText('${info.Address}')" class="btn-bounce bg-amber-500 mt-3">
